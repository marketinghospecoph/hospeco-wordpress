...
class WC_Points_Rewards_Manage_Points_List_Table extends WP_List_Table {
    ...
    $columns = array(
        ...
        'account_number' => __( 'Account Number', 'your-text-domain' ),
        'update'         => __( 'Update', 'woocommerce-points-and-rewards' ),
    );
    return $columns;
}
...
public function column_default( $user_points, $column_name ) {
    ...
    switch ( $column_name ) {
        case 'customer':
            ...
            $column_content = sprintf( '<a href="%s">%s</a>', admin_url( 'user-edit.php?user_id=' . $user_points->ID ), $customer_email );
            break;
        case 'points':
            $column_content = $points_balance;
            break;
        case 'account_number':
            $column_content = get_user_meta( $user_points->ID, 'my_account_field', true );
            break;
        case 'update':
            $column_content = sprintf( '<a href="%s">%s</a>', add_query_arg( array( 'action' => 'edit_user_points', 'user_id' => $user_points->ID ), admin_url( 'admin.php?page=wc_points_rewards_manage_points' ) ), __( 'Edit Points', 'woocommerce-points-and-rewards' ) );
            break;
        default:
            $column_content = '';
            break;
