...
class Troubleshooting {
	...
	public function __construct() {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			add_action(
				'wp_ajax_dgwt_wcas_troubleshooting_fix_outofstock',
				[
					$this,
					'ajaxFixOutOfStock__premium_only',
				]
			);
			add_action(
				'wp_ajax_dgwt_wcas_troubleshooting_switch_alternative_endpoint',
				[
					$this,
			...
			add_action(
				'wp_ajax_dgwt_wcas_troubleshooting_maintenance_analytics',
				[
					$this,
	...
	public function addSettingsTab( $settings ) {
		$settings[ self::SECTION_ID ] = apply_filters(
			'dgwt/wcas/settings/section=troubleshooting',
			[
				10 => [
	 ...
	 */
	public function ajaxFixOutOfStock__premium_only() {
		if ( ! current_user_can( Helpers::shopManagerHasAccess() ? 'manage_woocommerce' : 'manage_options' ) ) {
			wp_die( - 1, 403 );
		}
	...
	public function asyncActionHandler() {
		...
		switch ( $internalAction ) {
			...
			case 'regenerate_images':
				...
				$data['args'] = [ 'dgwt-wcas-regenerate-images-started' => true ];
				$success      = true;
				break;
		}
		...
		if ( ! empty( $tests['direct'] ) && is_array( $tests['direct'] ) ) {
			foreach ( $tests['direct'] as $test ) {
				if ( is_string( $test['test'] ) ) {
					...
					if ( method_exists( $this, $testFunction ) && is_callable( [ $this, $testFunction ] ) ) {
						$localize['troubleshooting']['tests']['direct'][] = $this->performTest(
							[
								$this,
								$testFunction,
							]
	...
	public function getTestLoopbackRequests() {
		...
		if ( $markAsCritical ) {
			...
			if ( dgoraAsfwFs()->is__premium_only() ) {
				...
				$result['description'] .= '<p>' . __(
					'The Indexer uses the WordPress function <code>wp_remote_post()</code> to build the index in the background. Sometimes the server can block this kind of request and responses with HTTP 401 Unauthorized or 403 Forbidden errors.',
			...
			$result['description'] .= '<p>' . sprintf(
				__(
					'Go to <a href="%s" target="_blank">Tools -> Site Health</a> in your WordPress. You should see issues related to REST API or Loopback request. Expand descriptions of these errors and follow the instructions. Probably you will need to contact your hosting provider to solve it.',
					'ajax-search-for-woocommerce'
				),
				$linkToWpHealth
			) . '</p>';
	...
	public function getTestAjaxAddToCart() {
		...
		if (
			...
			$result['description'] .= '<p>' . sprintf( __( 'Go to <code>WooCommerce -> Settings -> <a href="%1$s" target="_blank">Products (tab)</a></code> and check option <code>%2$s</code> and uncheck option <code>%3$s</code>.', 'ajax-search-for-woocommerce' ), $settingsUrl, $ajaxAtcLabel, $redirectLabel ) . '</p>';
	...
	public function getTestElementorSearchResultsTemplate() {
		...
		$dismissButton = get_submit_button( __( 'Dismiss', 'ajax-search-for-woocommerce' ), 'secondary', 'dgwt-wcas-async-action-dismiss-elementor-template', false, [ 'data-internal-action' => 'dismiss_elementor_template' ] );
		...
		$result['description'] .= '<p>' . sprintf(
			__(
				'Add <strong>Archive Products</strong> widget to the template <strong>%1$s</strong> or create a new template dedicated to the WooCommerce search results page. Learn how to do it in <a href="%2$s" target="_blank">our documentation</a>.',
				'ajax-search-for-woocommerce'
			),
			$templateLink,
			$linkToDocs
	...
	public function getTestMinThemeVersion() {
		...
		if ( ! empty( $unsupportedTheme ) ) {
			...
			$result['description'] .= '<p>' . sprintf( __( 'We’ve detected that you’re using the %1$s theme, which we support, but your current version (%2$s) is not compatible with our integration. To activate and use our integration properly, please update the theme to at least version %3$s.', 'ajax-search-for-woocommerce' ), '<strong>' . $unsupportedTheme['name'] . '</strong>', '<strong>v' . $unsupportedTheme['currentVersion'] . '</strong>', '<strong>v' . $unsupportedTheme['minimumVersion'] . '</strong>' ) . '</p>';
		}
	...
	public function getTestMinPluginVersions() {
		...
		if ( ! empty( $unsupportedPlugins ) ) {
			...
			foreach ( $unsupportedPlugins as $unsupportedPlugin ) {
				$result['description'] .= '<li>' . sprintf( __( '%1$s, current version: %2$s, minimum version: %3$s', 'ajax-search-for-woocommerce' ), '<strong>' . $unsupportedPlugin['name'] . '</strong>', '<strong>v' . $unsupportedPlugin['currentVersion'] . '</strong>', '<strong>v' . $unsupportedPlugin['minimumVersion'] . '</strong>' ) . '</li>';
			}
			$result['description'] .= '</ol>';
	...
	public function getTestProductFilterByWBW() {
		...
		foreach ( $filters as $filter ) {
			...
			$raw = is_array( $filter['setting_data'] )
				? $filter['setting_data']
				// phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.serialize_unserialize
				: unserialize( $filter['setting_data'] );
		...
		$result['label']        = sprintf(
			__( 'WBW Product Filter: Option %s is enabled', 'ajax-search-for-woocommerce' ),
			$option_label
		);
		$result['description']  = '<p>' . sprintf(
			__( 'We detected that Product Filter by WBW has the %s option enabled. With filtering active, this can cause filtering to return 0 results.', 'ajax-search-for-woocommerce' ),
			'<code>' . esc_html( $option_label ) . '</code>'
		) . '</p>';
		...
		$result['description'] .= '<li>' . sprintf(
			__( 'Turn off the %s option.', 'ajax-search-for-woocommerce' ),
			'<code>' . esc_html( $option_label ) . '</code>'
		) . '</li>';
	...
	public function getTestNotRegeneratedImages() {
		...
		$imagesBeforeActivation = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*)
			FROM $wpdb->posts
			WHERE post_type = 'attachment'
			...
			",
				$placeholderImage,
				wp_date( 'Y-m-d H:i:s', $activationDate )
			)
		);
		//phpcs:enable
		...
		if (
			...
			$dismissButton          = get_submit_button( __( 'Dismiss', 'ajax-search-for-woocommerce' ), 'secondary', 'dgwt-wcas-async-action-dismiss-regenerate-images', false, [ 'data-internal-action' => 'dismiss_regenerate_images' ] );
			$regenerateImagesButton = get_submit_button( __( 'Regenerate WooCommerce images', 'ajax-search-for-woocommerce' ), 'secondary', 'dgwt-wcas-async-action-regenerate-images', false, [ 'data-internal-action' => 'regenerate_images' ] );
...
EOT;
	...
	public function getTestIndexCompleteness__premium_only() {
		...
		if ( ! Builder::isIndexValid() ) {
			...
			$source   = new \DgoraWcas\Engines\TNTSearchMySQL\Indexer\SourceQuery( [ 'ids' => true ] );
	...
	public function getTestWooCommerceMultilingual__premium_only() {
		...
		if ( ! defined( 'WCML_VERSION' ) ) {
			...
			$result['description']   = '<p>' . sprintf(
				__( 'You are using the WPML Multilingual CMS and WooCommerce Multilingual plugins, but the latter requires action to be fully functional. Check <a href="%s" target="_blank">WooCommerce Multilingual status</a>.', 'ajax-search-for-woocommerce' ),
				$woocommerceWpmlAdminUrl
			) . '</p>';
		}
	...
	public function getTestNonCriticalIndexerErrors__premium_only() {
		...
		$result['description'] .= '<p>' . sprintf(
			__( 'If the following errors are related to your theme or plugins, try to fix them yourself or contact your developers. Otherwise, please <a href="%s" target="_blank">create a support ticket</a>.', 'ajax-search-for-woocommerce' ),
			dgoraAsfwFs()->contact_url()
		) . '</p>';
	...
	public function getTestOldIndex__premium_only() {
		...
		if ( ! Install::shouldRebuildIndex__premium_only( $version, DGWT_WCAS_VERSION ) ) {
			return $result;
		}
	...
	public function getTestAlternativeSearchEndpoint__premium_only() {
		...
		if ( $enabled ) {
			...
			$button = get_submit_button( __( "I've fixed it! Restore the default endpoint", 'ajax-search-for-woocommerce' ), 'secondary', 'dgwt-wcas-switch-alternative-endpoint', false, [ 'data-switch' => '0' ] );
	...
	public function getTestAnalyticsOptimization__premium_only() {
		...
		if ( $count > $recordsLimit ) {
			...
			$result['description'] .= '<p>' . sprintf( __( 'You have reached <b>%1$s records</b> in the <code>%2$s</code> table.', 'ajax-search-for-woocommerce' ), $recordsLimitLabel, $wpdb->dgwt_wcas_stats ) . '</p>';
	...
	public function getTestDetectionOfPluginsFoldersRenaming__premium_only() {
		...
		$renamedPlugins = apply_filters( 'dgwt/wcas/troubleshooting/renamed_plugins', [] );
		...
		if ( ! empty( $renamedPlugins ) ) {
			...
			foreach ( $renamedPlugins as $renamedPlugin ) {
				$result['description'] .= '<li>' . sprintf( __( '%1$s - %2$s', 'ajax-search-for-woocommerce' ), '<strong>' . $renamedPlugin['name'] . '</strong>', '<code>' . $renamedPlugin['folder'] . '</code>' ) . '</li>';
			}
			$result['description'] .= '</ol>';
	...
	public function getTestOutOfStockRelationships__premium_only() {
		...
		if ( $totalProducts > 0 && is_array( $outOfStockProductIds1 ) && is_array( $outOfStockProductIds2 ) ) {
			...
			if ( $diffPercent > 2 ) {
				...
				$result['description'] .= '<p>' . __(
					'We\'ve detected that some products with a status “Out of Stock“ may have something wrong with relationships in the database. It affects visibility of products in the search engine. This error is often the result of migrating from a different WordPress.',
					'ajax-search-for-woocommerce'
				) . '</p>';
				$result['description'] .= '<p>' . sprintf( __( 'Total “out of stock“ products calculated by SQL query on <code>%1$s</code> and <code>%2$s</code>: <b>%3$d</b>', 'ajax-search-for-woocommerce' ), $wpdb->posts, $wpdb->postmeta, count( $outOfStockProductIds1 ) ) . '</p>';
				$result['description'] .= '<p>' . sprintf( __( 'Total “out of stock“ products calculated by SQL query on <code>%1$s</code>: <b>%2$d</b>', 'ajax-search-for-woocommerce' ), $wpdb->term_relationships, count( $outOfStockProductIds2 ) ) . '</p>';
	...
	public function getTestSlowGettingOfSearchableCustomFields__premium_only() {
		...
		if ( time() - $start >= 10 ) {
			...
			$result['description'] .= '<p>' . __(
				'Once this SQL query is disabled, the "Search in custom fields" option will no longer display any available custom fields. If you want to search by custom fields, you must define the list of searchable custom fields manually.',
				'ajax-search-for-woocommerce'
			) . '</p>';
...
PHP;
	...
	public function getTestPingEndpoint__premium_only() {
		...
		$docUrl = 'https://fibosearch.com/documentation/troubleshooting/the-search-endpoint-url-is-blocked/';
		$button = get_submit_button( __( 'Enable alternative search endpoint', 'ajax-search-for-woocommerce' ), 'secondary', 'dgwt-wcas-switch-alternative-endpoint', false, [ 'data-switch' => '1' ] );

		$result['label']        = __( "The server returns an incorrect response for the search engine's AJAX calls", 'ajax-search-for-woocommerce' );
		$result['description'] .= '<p>' . sprintf( __( 'Server response with message <code>%1$s</code> and status code <code>%2$s</code>.', 'ajax-search-for-woocommerce' ), esc_html( $rMessage ), $rCode ) . '</p>';
		...
		$result['description'] .= '<li style="line-height: 30px;">' . sprintf(
			__(
				'Check how to solve this issue in <a target="_blank" href="%1$s">our documentation.</a> Before you fix the problem, switch on the alternative search endpoint here: %2$s. This will allow the search engine to work again, but search speed may not be optimal. Activate the alternative endpoint only temporarily, and solve the source of the problem as soon as possible.',
				'ajax-search-for-woocommerce'
			),
			$docUrl,
			$button
		) . '<span class="dgwt-wcas-ajax-loader"></span></li></ol>';
	...
	public function getTestDefenderSecurity__premium_only() {
		...
		if ( class_exists( 'WP_Defender_Free' ) && is_file( $htaccessPath ) ) {
			...
			if ( strpos( $htaccessContent, '## WP Defender - Protect PHP Executed ##' ) !== false ) {
				...
				$result['description']  = '<p><b>' . __( 'Solution', 'ajax-search-for-woocommerce' ) . '</b></p>';
				$result['description'] .= '<p>' . sprintf(
					__( 'Go to <code>Defender -> Security Tweaks -> <a href="%1$s" target="_blank">Resolved</a> -> Prevent PHP execution</code> and add following file path as exceptions: <code>%2$s</code>', 'ajax-search-for-woocommerce' ),
					$defenderSettingsUrl,
					$endpointPath
				) . '</p>';
			}
	...
	public function getTestNginx__premium_only() {
		...
		if ( strpos( $_SERVER['SERVER_SOFTWARE'], 'nginx' ) !== false ) {
			...
			$result['description'] .= '<p>' . sprintf(
				__(
					"Some NGINX configurations may block executing PHP files included directly in the plugin's directory. There is no one solution. It depends on your NGINX configuration. We recommend you contact your hosting provider and ask for the following file to be run: %s",
					'ajax-search-for-woocommerce'
				),
				$endpointUrl
			) . '</p>';
...
EOT;
	...
	public function getTestIndexerError__premium_only() {
		...
		$sendButton    = get_submit_button(
			__( 'Send the report', 'ajax-search-for-woocommerce' ),
			'primary',
			'dgwt-wcas-async-action-send-indexer-failure-report',
			false,
			[
		...
		$dismissButton = get_submit_button(
			_x( 'dismiss', 'Whole phrase: If you do not want to receive such notifications, you can {DISMISS} them.', 'ajax-search-for-woocommerce' ),
			'secondary',
			'dgwt-wcas-async-action-dismiss-indexer-failure-notices',
			false,
			[
		...
		$result['description'] .= ' ' . sprintf( _x( 'You can preview %1$s and %2$s that will be sent as an attachment.', 'There are links. The first: "the content of the report" and second "the logs".', 'ajax-search-for-woocommerce' ), $previewLink, $logsLink );
	...
	/**
	 * Test if products are missing in the searchable index
	 *
	 * @return array The test result.
	 */
	public function getTestMissingProductsInSearchableIndex__premium_only() {
		global $wpdb;
		...
		$result = [
			...
			'description' => '',
			'actions'     => '',
			'test'        => 'MissingProductsInSearchableIndex__premium_only',
		];
		...
		if ( ! empty( $lang ) ) {
			...
			$readableIds = $wpdb->get_col( $wpdb->prepare( "SELECT post_id FROM {$readableTableName} WHERE type = 'product' AND lang = %s", $lang ) );
		} else {
			//phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		...
		// More than 2% of products are missing.
		if ( ! empty( $missingIds ) && count( $missingIds ) / count( $readableIds ) > 0.02 ) {
			$docUrl = 'https://fibosearch.com/documentation/troubleshooting/the-search-index-could-not-be-built/#too-large-products-sets-to-process';

			$result['status']       = 'critical';
			$result['label']        = __( 'Some products are missing in the index', 'ajax-search-for-woocommerce' );
			$result['description']  = '<p>' . __( 'We have detected that more than 2% of your products are missing in the searchable index. This causes some products not to appear in search results.', 'ajax-search-for-woocommerce' ) . '</p>';
			$result['description'] .= '<p>' . __( 'List of missing products:', 'ajax-search-for-woocommerce' ) . '</p>';
			$result['description'] .= '<ol>';
			$i                      = 1;
			foreach ( $missingIds as $missingId ) {

				if ( $i > $limit ) {
					$result['description'] .= '<li style="list-style:none">' . sprintf( __( 'and %d other products...', 'ajax-search-for-woocommerce' ), count( $missingIds ) - $limit ) . '</li>';
					break;
				}

				$result['description'] .= '<li>' . sprintf( '<a href="%s" target="_blank">%s (#%s)</a>', esc_url( get_edit_post_link( $missingId ) ), esc_html( get_the_title( $missingId ) ), esc_html( $missingId ) ) . '</li>';
				$i ++;
			}
			$result['description'] .= '</ol>';
			$result['description'] .= '<p>' . __( 'You can try the solutions below:', 'ajax-search-for-woocommerce' ) . '</p>';
			$result['description'] .= '<ol>';
			$result['description'] .= '<li>' . sprintf( __( 'Process a smaller number of product sets <a href="%s" target="_blank">according to the documentation</a>.', 'ajax-search-for-woocommerce' ), $docUrl ) . '</li>';
			$result['description'] .= '<li>' . sprintf( __( 'Add a constant <code>%s</code> to <code>wp-config.php</code> to reduce server load during index rebuilding.', 'ajax-search-for-woocommerce' ), "define('DGWT_WCAS_INDEXER_MODE', 'sync');" ) . '</li>';
			$result['description'] .= '</ol>';
			$result['description'] .= '<p>' . sprintf( __( 'Is the problem still occurring? Write a <a target="_blank" href="%s">support request</a>.', 'ajax-search-for-woocommerce' ), dgoraAsfwFs()->contact_url() ) . '</p>';
	...
	public static function getTests() {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			...
			$tests['direct'][] = [
				'label' => __( 'Missing products in searchable index', 'ajax-search-for-woocommerce' ),
				'test'  => 'MissingProductsInSearchableIndex__premium_only',
			];
