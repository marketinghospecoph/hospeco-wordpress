...
class DetailsBox {
	...
	private function getProductDetails( $productID, $variationID = 0 ): array {
		...
		if ( ! $product->canUserSee( true, 'details-panel' ) ) {
			return $details;
		}
		...
		if (
			( $product->isType( 'simple' ) || $product->isType( 'variation' ) )
	...
	/**
	 * Prepare details for a non-product post type (Premium only).
	 *
	...
	private function getPostTypeDetails__premium_only( $postID ) {
		...
		// Allow only post types explicitly enabled for details panel (non-products).
		$allowedPostTypes = Helpers::getAllowedPostTypes( 'no-products' );
		if ( empty( $postType ) || ! in_array( $postType, $allowedPostTypes, true ) ) {
			return '';
		}
		...
		return (string) apply_filters( 'dgwt/wcas/suggestion_details/' . $postType . '/html', $html, $postID, $postType );
	}
