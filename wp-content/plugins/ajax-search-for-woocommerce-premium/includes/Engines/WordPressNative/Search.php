...
class Search {
	...
	public function __construct() {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			...
			add_filter(
				'dgwt/wcas/search_results/output',
				function ( $output ) {
		...
		add_filter( 'posts_search', [ $this, 'searchFilters' ], 501, 2 );
		add_filter( 'posts_where', [ $this, 'fixWooExcerptSearch' ], 100, 2 );
		add_filter( 'posts_distinct', [ $this, 'searchDistinct' ], 501, 2 );
		add_filter( 'posts_join', [ $this, 'searchFiltersJoin' ], 501, 2 );
		...
		add_action(
			...
			function () {
				if ( apply_filters( 'dgwt/wcas/override_search_results_page', true ) && ! Helpers::isNoFiboSearchModeActive() ) {
					...
					add_filter( 'posts_search', [ 'DgoraWcas\Helpers', 'clearSearchQuery' ], 1000, 2 );
					add_filter( 'the_posts', [ 'DgoraWcas\Helpers', 'rollbackSearchPhrase' ], 1000, 2 );
					add_filter( 'dgwt/wcas/search_page/result_post_ids', [ $this, 'getProductIds' ], 10, 2 );
				}
			}
		);
		...
		if ( Multilingual::isPolylang() && ! class_exists( 'Polylang_Woocommerce' ) && ! defined( 'Hyyan_WPI_DIR' ) ) {
			add_filter( 'woocommerce_ajax_get_endpoint', [ $this, 'fixPolylangWooEndpoint' ], 10, 2 );
		}
	...
	public function getSearchResults( $phrase = '', $return = false, $context = 'autocomplete' ) {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			...
			if ( isset( $_GET['dgwt_wcas_product_ids_only'] ) && $_GET['dgwt_wcas_product_ids_only'] == '1' ) {
				$context = 'product-ids-ff';
			}
		}
		...
		if ( $this->hasResults() ) {
			...
			if (
				! empty( $this->groups['product']['results'] ) &&
				( $showMoreProductsLink || count( $this->groups['product']['results'] ) < $totalProducts )
			) {
				$results[] = [
					'value' => '',
					'total' => $totalProducts,
			...
			if ( $i < $limit ) {
				...
				if ( $pos !== false ) {
					...
					$results['items'][ $i ] = [
						...
						'breadcrumbs' => Helpers::getTermBreadcrumbs( $cat->term_id, 'product_cat', [], $termLang, [ $cat->term_id ] ),
						'type'        => 'taxonomy',
					];
