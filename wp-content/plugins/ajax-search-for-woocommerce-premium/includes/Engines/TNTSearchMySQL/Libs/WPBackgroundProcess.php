...
abstract class WPBackgroundProcess extends WPAsyncRequest {
	...
	public function dispatch() {
		...
		if ( $redispatchProcess ) {
			for ( $i = 1; $i <= 10; $i ++ ) {
				...
				if ( ! in_array( Builder::getInfo( 'status', Config::getIndexRole() ), [ 'building' ] ) ) {
					return;
				}
			}
	...
	public function maybe_handle() {
		...
		Builder::log( $this->name . ' Maybe handle process', 'debug', 'file', 'bg-process', $this->action );

		if ( $this->is_process_running() ) {
			Builder::log( $this->name . ' Maybe handle process. Stop: background process already running', 'debug', 'file', 'bg-process', $this->action );
			// Background process already running.
			wp_die();
		}

		if ( $this->is_queue_empty() ) {
			Builder::log( $this->name . ' Maybe handle process. Stop: no data to process', 'debug', 'file', 'bg-process', $this->action );
			// No data to process.
			wp_die();
		}
		...
		if ( ! $check_nonce ) {
			Builder::log( $this->name . ' Maybe handle process. Stop: invalid nonce', 'debug', 'file', 'bg-process', $this->action );
	...
	protected function handle() {
		...
		Builder::log( $this->name . ' Locking and handling process', 'debug', 'file', 'bg-process', $this->action );
		...
		if ( ! $this->is_queue_empty() ) {
			Builder::log( $this->name . ' Process handled and unlocked. Queue is not empty. Dispatching next process', 'debug', 'file', 'bg-process', $this->action );
			$this->dispatch();
		} else {
			Builder::log( $this->name . ' Process handled and unlocked. Queue is empty. Complete', 'debug', 'file', 'bg-process', $this->action );
			$this->complete();
		}
	...
	protected function memory_exceeded() {
		...
		if ( $current_memory >= $memory_limit ) {
			Builder::log( $this->name . ' Batch memory exceeded: ' . size_format( $current_memory ) . '/' . size_format( $memory_limit ), 'debug', 'file', 'bg-process', $this->action );
			$return = true;
		}
	...
	protected function time_exceeded( $log = false ) {
		...
		if ( time() >= $finish ) {
			if ( $log ) {
				Builder::log( $this->name . ' Batch time exceeded: ' . ( time() - $this->start_time ) . '/' . apply_filters( $this->identifier . '_default_time_limit', 20 ) . 's', 'debug', 'file', 'bg-process', $this->action );
			}
			$return = true;
		}
	...
	public function handle_cron_healthcheck() {
		...
		if ( Builder::isIndexerWorkingTooLong( 80 ) ) {
			Builder::log( $this->name . ' Handle process via cron healthcheck', 'debug', 'file', 'bg-process', $this->action );
			$this->call_source = 'cron';
			$this->handle();
		}
