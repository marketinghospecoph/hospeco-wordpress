...
class Helpers {
	...
	public static function getIcon( $name, $class = '', $color = '' ) {
		...
		switch ( $name ) {
			case 'magnifier-thin':
				...
				?>
				<svg
					class="<?php echo $class; ?>" xmlns="http://www.w3.org/2000/svg"
			...
			case 'magnifier-pirx':
				...
				?>
				<svg
					class="<?php echo $class; ?>" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18">
			...
			case 'voice-search-active':
				...
				?>
				<svg
					class="<?php echo $class; ?>" xmlns="http://www.w3.org/2000/svg" height="24"
					width="24">
					<path <?php echo $style; ?>
	...
	public static function getSettingsProLabel( $label, $type = 'header', $headerSubtitle = '' ) {
		...
		switch ( $type ) {
			case 'header':
				...
				$html .= '<div class="dgwt-wcas-row dgwt-wcas-pro-header"><span class="dgwt-wcas-pro-label">' . $label . '</span><span class="dgwt-wcas-pro-suffix">' . __(
					'Pro',
					'ajax-search-for-woocommerce'
				) . '</span></div>';
				break;
			case 'option-label':
				$html .= '<div class="dgwt-wcas-row dgwt-wcas-pro-field"><span class="dgwt-wcas-pro-label">' . $label . '</span><span class="dgwt-wcas-pro-suffix">' . __(
					'Pro',
					'ajax-search-for-woocommerce'
				) . '</span></div>';
				break;
	...
	public static function getTermBreadcrumbs( $termID, $taxonomy, $visited = [], $lang = '', $exclude = [] ) {
	...
	public static function isTableExists( $tableName ) {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			if ( ! self::isIndexing__premium_only() ) {
	...
	public static function getOverrideOptionText( $theme ) {
		...
		$content             = '<p>' . sprintf(
			__(
				'This option is <b>overridden</b> by the seamless integration with the %s theme. If you want to change the value of this option, disable the integration in <br /><b>WooCommerce -> FiboSearch -> Starting (tab)</b>.',
				'ajax-search-for-woocommerce'
			),
			$theme
		) . '</p>';
		$content            .= '<p>' . sprintf(
			__(
				'Furthermore, you can override this option for a specific search bar via shortcode params. <a href="%s" target="_blank">Learn more about shortcodes parameters</a>.',
				'ajax-search-for-woocommerce'
			),
			$linkToShortcodesDoc
		) . '</p>';
	...
	public static function getScriptsSettings() {
		...
		if ( dgoraAsfwFs()->is__premium_only() ) {
			...
			if ( is_array( $variationSupportModes ) && in_array( 'exact_match', $variationSupportModes ) && ! in_array( 'as_single_product', $variationSupportModes ) ) {
	...
	public static function searchProducts( $phrase ) {
		if ( doing_action( 'pre_get_posts' ) ) {
			_doing_it_wrong(
				__METHOD__,
				'You should not call Helpers::searchProducts() during pre_get_posts. Collect post IDs earlier.',
				'0.1.1'
			);
		}
		$postIn  = [];
	...
	public static function getFilterGroupTerms__premium_only( $group, $values ) {
		...
		if ( Multilingual::isMultilingual() ) {
			...
			$terms = Multilingual::getTermsInLang(
				[
					'taxonomy'   => $taxonomy,
					'include'    => $values,
					'hide_empty' => false,
...
AND    post_status = 'publish'
	 ...
	 */
	public static function fixOutOfStock__premium_only() {
		global $wpdb;
	...
	public static function isNoFiboSearchModeActive() {
		return isset( $_GET['nofibosearch'] ) && $_GET['nofibosearch'] === '1' // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					|| apply_filters( 'dgwt/wcas/nofibosearch', '' ) === '1';
	}
	...
	public static function maybeInjectFiboDebugFields( string $key ): void {
		...
		add_action(
			...
			function () {
				printf(
					'<input type="hidden" name="fibodebug" value="%s"/>',
					esc_attr( $_GET['fibodebug'] )
				);
			}
		);

		add_filter(
			'dgwt/wcas/search_bar/custom_params',
			function ( $params ) {
