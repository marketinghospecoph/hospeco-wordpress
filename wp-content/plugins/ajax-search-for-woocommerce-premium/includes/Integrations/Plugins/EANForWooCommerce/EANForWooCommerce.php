...
class EANForWooCommerce extends AbstractPluginIntegration {
	...
	public function init(): void {
		...
		if ( ! is_admin() ) {
			if (
				isset( alg_wc_ean()->core->search ) &&
				get_option( 'alg_wc_ean_frontend_search', 'no' ) === 'yes'
			) {
				...
				if ( ! dgoraAsfwFs()->is_premium() ) {
					...
					add_filter( 'dgwt/wcas/native/search_query/search_or', [ $this, 'searchQueryOr' ], 10, 2 );
				}
			}
		}

		if ( dgoraAsfwFs()->is__premium_only() ) {
			// Add EAN to the list of custom fields for indexing.
			add_filter(
				'dgwt/wcas/indexer/source_query/search_in_custom_fields',
				[
					$this,
					'searchInCustomFields__premium_only',
				]
			);
		}
	}
	...
	public function searchInCustomFields__premium_only( $customFields ) {
		if ( get_option( 'alg_wc_ean_frontend_search', 'no' ) !== 'yes' ) {
			return $customFields;
		}
		if ( isset( alg_wc_ean()->core->ean_key ) ) {
			$customFields[] = alg_wc_ean()->core->ean_key;
		}
