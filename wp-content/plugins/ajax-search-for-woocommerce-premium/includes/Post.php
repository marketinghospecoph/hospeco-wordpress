...
class Post {
	...
	/**
	 * Determine whether the current user can access post details in the details panel.
	 *
	 * @return bool
	 */
	public function canUserSee(): bool {
		if ( ! $this->isValid() ) {
			return false;
		}
		...
		$canShow = ( $status === 'publish' );

		// Non-public posts: allow only authorized users.
		if ( ! $canShow ) {
			$canShow = current_user_can( 'read_post', $postId );
		}

		// Do not reveal password-protected content to public users.
		if ( $canShow && post_password_required( $this->post ) && ! current_user_can( 'edit_post', $postId ) ) {
			$canShow = false;
		}

		return (bool) apply_filters( 'dgwt/wcas/post/can_show_in_details_panel', $canShow, $this );
	}
